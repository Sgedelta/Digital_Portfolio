...
let stopCyclingPhone = {val: true, running: false};
const cycleRandomPhoneChars = async () => {

    stopCyclingPhone.running = true;

    //setup!
    let delay = 25; //delay in ms
    const currDisplay = lodash.unescape(phoneDisplay.textContent);
    let newDisplay = "";
    if(phoneDisplay.classList.contains("mouseOver")) {
        const protectedPhone = "(S5P1A3M)P R8O8T2E C-T I6O3N6S6P";
        //construct the real phone out of a spam protected one
        const doubleLen = protectedPhone.length;
        for(let i = 0; i < doubleLen; i+= 2) {
            newDisplay = newDisplay.concat(protectedPhone.charAt(i));
        }
        delay = 40;
        stopCyclingPhone.val = false; //stop future cycles
    } else {
        newDisplay = "(" + getRandomChars(3) + ") " + getRandomChars(3) + " - " + getRandomChars(4);; //generates a new random junk email.
    }
    ...
    //Phone is a constant length, so just loop through replacing at random positions
    while(visited.length > 0) {
        //get a random index of visited, then remove it from visited.
        const randomIndex = Math.floor(Math.random() * visited.length);
        const randomNumber = visited[randomIndex];
        ...
        //set textContent to new char
        phoneDisplay.textContent = 
            lodash.unescape(phoneDisplay.textContent).substring(0, randomNumber)
            .concat(newDisplay.charAt(randomNumber))
            .concat(lodash.unescape(phoneDisplay.textContent).substring(randomNumber+1));
    ...
    stopCyclingPhone.running = false;
...
const contactInfoSetup = () => {
        ...
        for(const item of mutationList) {
            if(item.attributeName === "class") {
                ...
                if(!classList.contains("hidden")) {
                    ...
                    stopCyclingPhone.val = true;
                    repeatFunctionUntilFalse(stopCyclingEmail, cycleRandomEmailChars);
                    repeatFunctionUntilFalse(stopCyclingPhone, cycleRandomPhoneChars);
                } 
                else {
                    stopCyclingEmail.val = false;
                    stopCyclingPhone.val = false;
                }
            }
        }
    });
    ...
    //same with phone
    phoneDisplay.addEventListener('mouseover', mouseOverDetect);
    phoneDisplay.addEventListener('mouseout', (e) => {
        mouseOutDetect(e);
        stopCyclingPhone.val = true;
        repeatFunctionUntilFalse(stopCyclingPhone, cycleRandomPhoneChars);
    });
